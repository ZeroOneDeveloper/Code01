...
async def run_code_in_background(
    ...
    try:
        ...
        time_limit_ms = (
            20000
            if problem[0].get("time_limit") is None
            else problem[0].get("time_limit")
        )
        memory_limit_mb = (
            128
            if problem[0].get("memory_limit") is None
            else problem[0].get("memory_limit")
        )
        ...
        try:
            await supabase.table("problem_submissions").update(
                {"status_code": 7, "stderr_list": [str(e)]}  # InternalError
            ).eq("id", pendingId).execute()
        except Exception as db_e:
            logging.error(
                f"Failed to update DB for submission {pendingId} on error: {db_e}"
            )
