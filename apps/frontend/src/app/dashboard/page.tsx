      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8">
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.title} className="bg-white dark:bg-gray-800/50 rounded-xl shadow-md overflow-hidden transition-transform hover:scale-105 duration-300">
                <div className="p-5">
                  <div className="flex items-center">
                    <div className={`flex-shrink-0 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg ${stat.color}`}>
                  ...
                  <div className="mt-4 text-xs text-gray-500 dark:text-gray-400">{stat.change}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="lg:col-span-2 bg-white dark:bg-gray-800/50 rounded-xl shadow-md p-6">
              ...
              <div className="flex items-end justify-around h-64 w-full space-x-2">
                {submissionChartData.map((data) => (
                  <div key={data.name} className="flex flex-col items-center space-y-2 w-full">
                    <div
            ...
            <div className="bg-white dark:bg-gray-800/50 rounded-xl shadow-md p-6">
              ...
              <ul className="space-y-4">
                {recentSubmissions.map((submission) => (
                  <li key={submission.id} className="flex items-center space-x-3">
                    <div className={`flex-shrink-0 p-2 rounded-full ${submission.status === 'Accepted' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                      {submission.status === 'Accepted' ? <CheckCheck className="h-5 w-5" /> : <Code className="h-5 w-5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{submission.problem}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 truncate">by {submission.user}</p>
                    </div>
